%% demo_A2B2A.m
% Simulate A-Format to B-Format encoding and back to A-Format 
...
nsamples = size(sim_rir,2);

b_n_len = P.hoa_encoding.b_n_len;
[b_n_in_time, b_n_inverse_in_time] = compute_bn_fir_filters(sh_order, fs, b_n_len, c, rmic, isRigid);

%% B-Format Encoding
Y = SHTools.getRealSH(sh_order, mic_el, mic_az);
S = pinv(Y');
rir_SH = tensorprod(S,sim_rir,2,1);
